...
import '../../state/auth/providers/registration_providers.dart';

class RegistrationOtpVerificationPage extends StatelessWidget {
  const RegistrationOtpVerificationPage({Key? key}) : super(key: key);
    ...
    return Scaffold(
      body: SafeArea(
          child: FractionallySizedBox(
              widthFactor: 1, child: RegistrationPinPut())),
    );
  }
}

class RegistrationPinPut extends ConsumerWidget {
  RegistrationPinPut({super.key});
              ...
              Directionality(
                ...
                child: Pinput(
                  ...
                  validator: (value) {
                    ...
                    if (userToken != null) {
                      devtools.log(
                          "${ConstantsLog.fromRegistrationOtpPage} user token update to: $userToken");
                      return null;
                    } else {
                      devtools.log(
                          "${ConstantsLog.fromRegistrationOtpPage} user token still null");
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (isLoading == false && pinController.text.isNotEmpty) {
                        ...
                        ref
                            .watch(registrationStep2PayloadProvider.notifier)
                            .state = RegistrationStep2Payload(
                          step1Id: step2Id,
                          smsCode: otpCode,
                        );

                        final step2Payload = ref
                            .read(registrationStep2PayloadProvider.notifier)
                            .state;

                        devtools.log(step2Payload.toString());

                        final step2Response =
                            await ref.watch(registrationStep2Provider.future);
                        ref
                            .read(loadingNotifierProvider.notifier)
